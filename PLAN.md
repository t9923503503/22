...
- [x] Извлечь `loadHistory()` / `saveHistory()` хелперы — 11 чтений + 2 записи → `main.js` — 2026-03-20
...
- [~] `home.js` — хардкод "Epic Player Card" (MAMEDOV / РАНГ: 3850) — не нужно, оставить как есть
...
- [x] `index.html` — `<label class="sr-only">` для обоих полей пароля; `.sr-only` добавлен в `app.css` — 2026-03-20
- [x] `index.html` — `<noscript>` fallback добавлен — 2026-03-20
...
**Осталось незакрытых задач:** ~7 из 73
- Фаза 4: 1 задача (валидация телефона — низкий приоритет, частный клуб)
- Фаза 5: ~5 задач (рефактор архитектуры — можно делать, unit-тесты есть)
- Фаза 6: 1 задача (PNG иконки требуют copy_logo.js)
